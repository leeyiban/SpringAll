...
Model、ModelMap、Map类型的参数其实***本质上都是 BindingAwareModelMap 类型***的
...
### 3、具体功能：访问首页
#### 代码写到这里的时候可能会出现vue找不到文件的情况
######出现这个的原因 1.maven打包的时候没有吧vue.js引进来 2.Springmvc会把静态资源给拦截处理,需要在Springmvc的配置文件中加上default-servlet-handler注解,当springmvc解析不了的时候,交给普通的servlet来处理
...
<mvc:interceptors>
    ...
    <ref bean="firstInterceptor"></ref><!--这种方式是需要在类上加上@compent  让Spring扫描到容器中，才能在这里应用到！-->
...
将这个类写到WebInit类中,通过重写的getRootConfigClasses方法,将这个类注册到容器中
```java
@Configuration
...
同理也是,通过重写的webInit类中重写的getServletConfigClasses方法,注册到容器中
```java
@Configuration
//扫描组件
